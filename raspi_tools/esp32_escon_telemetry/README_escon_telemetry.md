...
## 2026-07-20 Stepper Current Pin Note

Stepper current telemetry currently follows the actual robot wiring:

```text
SLA7078MPRT SenseA -> 1 kOhm -> ESP32 GPIO27, 0.33 uF to GND
SLA7078MPRT SenseB -> 1 kOhm -> ESP32 GPIO32, 0.33 uF to GND
```

Do not use GPIO33 for stepper current because GPIO33 is used by `ACCE1_X`.
The old setting shared GPIO33 between `step_sense_a_v` and `acce1_x_v`, so the
dashboard converted accelerometer voltage into a false stepper current value
such as 10 A.

GPIO27 is ADC2.  It can be blocked or noisy while Wi-Fi is active, so this
firmware turns Wi-Fi off after the OTA window.  Wait about 30 seconds after
reset before trusting `step_sense_a_v`.
...
The `step_sense_*_a` values are estimates.  During tuning, trust
`step_sense_*_v` first and calibrate the voltage-to-current conversion against
a tester or power-supply reading.
