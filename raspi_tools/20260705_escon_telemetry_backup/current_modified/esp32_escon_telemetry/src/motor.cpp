...
void motor_update_telemetry() {
  ...
  int current_adc = analogRead(ESCON_CURRENT_ADC_PIN);
  s_current_v =
      static_cast<float>(current_adc) * ESP32_ADC_REF_V / ESP32_ADC_MAX_COUNTS;
  s_current_a = s_current_v * ESCON_CURRENT_A_PER_V + ESCON_CURRENT_A_OFFSET;

  int speed_adc = analogRead(ESCON_SPEED_ADC_PIN);
  s_speed_v =
      static_cast<float>(speed_adc) * ESP32_ADC_REF_V / ESP32_ADC_MAX_COUNTS;
