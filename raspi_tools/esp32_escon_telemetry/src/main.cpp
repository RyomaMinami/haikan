...
static uint32_t s_otaStartedMs = 0;
#endif
...
static void serviceOtaWindow() {
  if (!s_otaReady) {
    return;
  }
  ArduinoOTA.handle();
  if (OTA_ACTIVE_WINDOW_MS > 0 && millis() - s_otaStartedMs > OTA_ACTIVE_WINDOW_MS) {
    s_otaReady = false;
    WiFi.disconnect(true);
    WiFi.mode(WIFI_OFF);
    Serial2.println("OTA,wifi_off_for_adc2");
  }
}
#endif
